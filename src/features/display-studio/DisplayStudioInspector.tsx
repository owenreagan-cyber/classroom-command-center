...
function WidgetsSection({ screen }: { screen: DisplayScreen }) {
  const updateScreen = useDisplayComposerStore((s) => s.updateScreen)
  const { toggleWidgetLibrary } = useDisplayStudioUI()
    ...
    <div className="space-y-2 px-3 pb-3">
      ...
      {!screen.materialsCard ? (
        <button
          ...
          onClick={() => updateScreen(screen.id, { materialsCard: { heading: 'Materials', sections: [{ id: 'sec-1', items: [] }] } })}
        >
          + Add Materials Card
        </button>
      ) : (
      ...
      {!screen.checklistCard ? (
        <button
          type="button"
          className={`w-full ${secondaryBtn}`}
          onClick={() => updateScreen(screen.id, { checklistCard: { heading: 'Checklist', items: [] } })}
        >
          + Add Checklist Card
        </button>
      ) : (
  ...
  return (
    <div className="space-y-2 px-3 pb-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        <button
          type="button"
          className="rounded-lg border border-cyan-400/50 bg-cyan-950/40 px-2 py-1 text-[10px] font-semibold text-cyan-100 transition hover:bg-cyan-900/50"
          onClick={() => {
...
export function DisplayStudioInspector() {
  ...
  const { selectedScreenId, expandedInspectorSections, toggleInspectorSection } = useDisplayStudioUI()
              ...
              <div key={id} className="border-b border-slate-800/50 py-1">
                ...
                {isExpanded && (
                  <>
                    ...
                    {id === 'teacher-notes' && <TeacherNotesSection />}
