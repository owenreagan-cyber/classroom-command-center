...
/**
 * Minimal Presenter Mode — current screen + next screen + private notes preview.
 * Shown as an overlay within the Display Studio (not a separate route).
 * Designed to mimic PowerPoint Presenter View: current slide, next slide, notes, controls.
 */
export function DisplayStudioPresenter() {
  const { presenterMode, togglePresenterMode } = useDisplayStudioUI()
  ...
  return (
    <div className="fixed inset-0 z-[60] flex bg-slate-950" data-display-studio-presenter>
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500 mb-1">Current Screen</p>
      ...
      <div className="flex w-72 shrink-0 flex-col border-l border-slate-800 p-6">
        ...
        {nextScreen && (
          <p className="mt-2 text-xs font-semibold text-slate-300">{nextScreen.title}</p>
        )}
        ...
        {currentScreen && (
          <div className="mt-4 flex-1 overflow-hidden">
            ...
            <textarea
              className="h-full w-full resize-none rounded-lg border border-slate-700 bg-slate-900/50 p-2 text-xs text-slate-300 placeholder:text-slate-600"
              placeholder="Private teacher notes — never shown on /display. Timing cues, reminders, group notes..."
              readOnly
              value=""
            ...
            {order.slice(0, 8).map((id) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    const { selectScreen } = useDisplayStudioUI as unknown as { selectScreen: (id: string) => void }
                    selectScreen(id)
                  }}
                >
                  {s.title.slice(0, 12)}
