import { useEffect, useMemo, useState } from 'react'
import { useBoardStore } from '../store/boardStore'
import { useTimerStore } from '../store/timerStore'
import { useAtmosphereStore } from '../features/classroom-atmosphere/atmosphereStore'
import { MUSIC_MODE_LABELS } from '../features/classroom-atmosphere/playlists'
import { formatTimerMs } from '../lib/timerFormat'
import type {
  HomeroomContent,
  MathContent,
  ReadingContent,
  SubjectContent,
  SnackContent,
  LunchContent,
  ReadyPositionContent,
} from '../data/types'

function formatTime(date: Date): string {
  return date.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })
}

function formatDate(date: Date): string {
  return date.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })
}

function useLiveClock() {
  const [now, setNow] = useState(() => new Date())
  useEffect(() => {
    const id = setInterval(() => setNow(new Date()), 30_000)
    return () => clearInterval(id)
  }, [])
  return now
}

/** A slide-section block — lighter and more open than a web-app card. */
function SlideSection({
  title,
  children,
}: {
  title?: string
  children: React.ReactNode
}) {
  return (
    <div className="rounded-3xl border border-white/[0.05] bg-white/[0.015] p-6 backdrop-blur-[2px]">
      {title && (
        <h3 className="mb-5 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500/70">
          {title}
        </h3>
      )}
      {children}
    </div>
  )
}

function BulletList({ items }: { items: string[] }) {
  if (!items || items.length === 0) return null
  return (
    <ol className="space-y-3">
      {items.map((item, i) => (
        <li key={i} className="flex items-start gap-3.5 text-base leading-relaxed text-slate-200/90">
          <span className="mt-[2px] shrink-0 text-sm font-medium text-slate-600">
            {i + 1}.
          </span>
          <span>{item}</span>
        </li>
      ))}
    </ol>
  )
}

function MaterialPill({ label }: { label: string }) {
  return (
    <span className="inline-flex items-center gap-2 rounded-full border border-white/[0.05] bg-white/[0.02] px-4 py-2 text-sm font-medium text-slate-300/80">
      <span className="text-[10px] text-slate-600">□</span>
      {label}
    </span>
  )
}

const VOICE_COLORS: Record<string, string> = {
  Silent: 'border-slate-600/30 text-slate-500',
  Whisper: 'border-cyan-600/30 text-cyan-300/60',
  Normal: 'border-emerald-600/30 text-emerald-300/60',
}

function VoiceBadge({ level }: { level: string }) {
  const colors = VOICE_COLORS[level] ?? 'border-slate-500/30 text-slate-500'
  return (
    <span
      className={`inline-flex shrink-0 items-center rounded-lg border px-2.5 py-1 text-[11px] font-medium ${colors}`}
    >
      {level}
    </span>
  )
}

function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <h4 className="mb-3 text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-600">
      {children}
    </h4>
  )
}

/* ── Screen-specific preview components ── */

function HomeroomPreview({ content }: { content: HomeroomContent }) {
  return (
    <div className="grid gap-5 md:grid-cols-[1.3fr_0.7fr]">
      <SlideSection title={content.doNowTitle || 'Morning Work'}>
        <p className="text-base leading-relaxed text-slate-200/90">{content.doNow}</p>
        {content.reminders && content.reminders.length > 0 && (
          <div className="mt-6 border-t border-white/[0.04] pt-6">
            <SectionHeading>{content.remindersTitle || 'Reminders'}</SectionHeading>
            <BulletList items={content.reminders} />
          </div>
        )}
      </SlideSection>
      <SlideSection title={content.materialsTitle || 'Materials'}>
        <div className="flex flex-col gap-3">
          {content.materials?.haveOut && content.materials.haveOut.length > 0 && (
            <div>
              <SectionHeading>Have Out</SectionHeading>
              <div className="flex flex-wrap gap-2">
                {content.materials.haveOut.map((m, i) => (
                  <MaterialPill key={i} label={m} />
                ))}
              </div>
            </div>
          )}
        </div>
      </SlideSection>
    </div>
  )
}

function SubjectPreview({ content }: { content: SubjectContent }) {
  return (
    <div className="grid gap-5 md:grid-cols-[1.3fr_0.7fr]">
      <SlideSection title={content.focusTitle || 'Focus'}>
        <p className="text-base leading-relaxed text-slate-200/90">{content.focusTask}</p>
        {content.agenda && content.agenda.length > 0 && (
          <div className="mt-6 border-t border-white/[0.04] pt-6">
            <SectionHeading>{content.agendaTitle || 'Agenda'}</SectionHeading>
            <BulletList items={content.agenda} />
          </div>
        )}
      </SlideSection>
      <SlideSection title={content.materialsTitle || 'Materials'}>
        <div className="flex flex-col gap-3">
          {content.materials?.haveOut && content.materials.haveOut.length > 0 && (
            <div>
              <SectionHeading>Have Out</SectionHeading>
              <div className="flex flex-wrap gap-2">
                {content.materials.haveOut.map((m, i) => (
                  <MaterialPill key={i} label={m} />
                ))}
              </div>
            </div>
          )}
        </div>
      </SlideSection>
    </div>
  )
}

function MathReadingPreview({ content }: { content: MathContent | ReadingContent }) {
  const lesson = content.lesson
  const vocabulary = content.vocabulary
  const ready = 'readyPosition' in content ? content.readyPosition : undefined

  return (
    <div className="grid gap-5 md:grid-cols-[1.3fr_0.7fr]">
      <div className="flex flex-col gap-5">
        <SlideSection title="Lesson">
          <p className="text-base font-medium leading-relaxed text-slate-100/90">
            {content.lessonTitle}
          </p>
          {lesson && (
            <div className="mt-5 space-y-4 border-t border-white/[0.04] pt-5">
              {lesson.objective && (
                <p className="text-sm leading-relaxed text-slate-400/80">
                  <span className="font-medium text-slate-400">Objective: </span>
                  {lesson.objective}
                </p>
              )}
              {lesson.successCriteria && lesson.successCriteria.length > 0 && (
                <div>
                  <SectionHeading>Success Criteria</SectionHeading>
                  <BulletList items={lesson.successCriteria} />
                </div>
              )}
            </div>
          )}
        </SlideSection>
        {vocabulary && vocabulary.entries.length > 0 && (
          <SlideSection title={vocabulary.title || 'Vocabulary'}>
            <div className="flex flex-wrap gap-1.5">
              {vocabulary.entries.map((entry, i) => (
                <span
                  key={i}
                  className="rounded-xl border border-white/[0.05] bg-white/[0.02] px-3.5 py-2 text-sm font-medium text-slate-300/80"
                >
                  {entry.term}
                </span>
              ))}
            </div>
          </SlideSection>
        )}
      </div>
      <div className="flex flex-col gap-5">
        <SlideSection title={content.materialsTitle || 'Materials'}>
          <div className="flex flex-col gap-3">
            {content.materials?.haveOut && content.materials.haveOut.length > 0 && (
              <div>
                <SectionHeading>Have Out</SectionHeading>
                <div className="flex flex-wrap gap-2">
                  {content.materials.haveOut.map((m, i) => (
                    <MaterialPill key={i} label={m} />
                  ))}
                </div>
              </div>
            )}
          </div>
        </SlideSection>
        {ready && (
          <SlideSection title={ready.title || 'Ready Position'}>
            {ready.useCompact && ready.compactLine ? (
              <p className="text-base text-slate-200/90">{ready.compactLine}</p>
            ) : (
              <BulletList items={ready.steps} />
            )}
          </SlideSection>
        )}
      </div>
    </div>
  )
}

function SnackLunchPreview({ content }: { content: SnackContent | LunchContent }) {
  return (
    <div className="grid gap-5 md:grid-cols-[1.3fr_0.7fr]">
      <SlideSection title={content.routineTitle || 'Routine'}>
        {content.routine && <BulletList items={content.routine} />}
      </SlideSection>
      <SlideSection title={content.cleanupTitle || 'Cleanup'}>
        {content.cleanupReminders && <BulletList items={content.cleanupReminders} />}
      </SlideSection>
    </div>
  )
}

function ReadyPositionPreview({ content }: { content: ReadyPositionContent }) {
  return (
    <SlideSection title={content.title || 'Expectations'}>
      {content.useCompact && content.compactLine ? (
        <p className="text-xl font-medium text-slate-200/90">{content.compactLine}</p>
      ) : content.steps && content.steps.length > 0 ? (
        <BulletList items={content.steps} />
      ) : null}
    </SlideSection>
  )
}

function EmptyScreenPreview({ screenLabel }: { screenLabel: string }) {
  return (
    <div className="flex h-full items-center justify-center">
      <p className="text-sm text-slate-600">{screenLabel} — no content configured yet.</p>
    </div>
  )
}

/** Clean, non-editable classroom screen preview — presentation-first, 16:9 slide feel. */
export function CleanClassroomScreenPreview() {
  const activeScreen = useBoardStore((s) => s.activeScreen)
  const contents = useBoardStore((s) => s.contents)
  const simpleTimers = useTimerStore((s) => s.simpleTimers)
  const activeMode = useAtmosphereStore((s) => s.activeMode)
  const isPlaying = useAtmosphereStore((s) => s.isPlaying)
  const now = useLiveClock()

  const screenLabel = useMemo(() => {
    switch (activeScreen) {
      case 'homeroom':
        return 'Morning Arrival'
      case 'ready-position':
        return 'Ready Position'
      default:
        return activeScreen.charAt(0).toUpperCase() + activeScreen.slice(1).replace(/-/g, ' ')
    }
  }, [activeScreen])

  const greeting = useMemo(() => {
    const hour = now.getHours()
    if (hour < 12) return 'Good Morning'
    if (hour < 17) return 'Good Afternoon'
    return 'Good Evening'
  }, [now])

  const timerRemaining = useMemo(() => {
    const timer = simpleTimers[activeScreen as keyof typeof simpleTimers]
    if (!timer || timer.status !== 'running') return null
    return formatTimerMs(timer.remainingMs)
  }, [simpleTimers, activeScreen])

  const musicLabel = useMemo(() => {
    if (!activeMode) return null
    return MUSIC_MODE_LABELS[activeMode] ?? activeMode
  }, [activeMode])

  const voiceLevel = useMemo(() => {
    const screenToLevel: Record<string, string> = {
      homeroom: 'Whisper',
      math: 'Silent',
      reading: 'Whisper',
      writing: 'Normal',
      science: 'Normal',
      'social-studies': 'Normal',
      spelling: 'Normal',
      assessment: 'Silent',
      centers: 'Whisper',
      snack: 'Normal',
      lunch: 'Normal',
      recess: 'Normal',
      movement: 'Normal',
      'ready-position': 'Silent',
      homework: 'Normal',
      'pack-up': 'Normal',
    }
    return screenToLevel[activeScreen] ?? null
  }, [activeScreen])

  const renderContent = () => {
    switch (activeScreen) {
      case 'homeroom':
        return <HomeroomPreview content={contents.homeroom} />
      case 'math':
        return <MathReadingPreview content={contents.math} />
      case 'reading':
        return <MathReadingPreview content={contents.reading} />
      case 'writing':
      case 'science':
      case 'social-studies':
      case 'assessment':
      case 'centers':
      case 'spelling':
        return <SubjectPreview content={contents[activeScreen] as SubjectContent} />
      case 'homework':
      case 'pack-up':
        return <SubjectPreview content={contents[activeScreen] as unknown as SubjectContent} />
      case 'snack':
        return <SnackLunchPreview content={contents.snack} />
      case 'lunch':
        return <SnackLunchPreview content={contents.lunch} />
      case 'recess':
        return <ReadyPositionPreview content={contents.recess} />
      case 'movement':
        return <ReadyPositionPreview content={contents.movement} />
      case 'ready-position':
        return <ReadyPositionPreview content={contents['ready-position']} />
      default:
        return <EmptyScreenPreview screenLabel={screenLabel} />
    }
  }

  return (
    <div className="flex h-full w-full items-center justify-center p-5 sm:p-8 md:p-10 lg:p-12">
      <div className="flex w-full max-w-4xl flex-col">
        {/* Slide card — Keynote/Canva presentation feel */}
        <div className="overflow-hidden rounded-[2rem] border border-white/[0.06] bg-gradient-to-b from-slate-900/80 to-slate-950/90 shadow-[0_0_0_1px_rgba(255,255,255,0.02),0_4px_48px_-12px_rgba(0,0,0,0.5),0_2px_12px_-4px_rgba(0,0,0,0.4)]">
          {/* Header strip — greeting + date + voice + clock */}
          <div className="border-b border-white/[0.04] bg-white/[0.01] px-8 py-6">
            <div className="flex items-start justify-between gap-8">
              <div className="min-w-0">
                <h2 className="text-[1.75rem] font-semibold tracking-tight text-white/95">
                  {greeting}, 4th Grade!
                </h2>
                <p className="mt-1.5 text-sm font-medium text-slate-500/80">{formatDate(now)}</p>
              </div>
              <div className="flex shrink-0 items-center gap-3">
                {voiceLevel && <VoiceBadge level={voiceLevel} />}
                <span className="rounded-xl bg-white/[0.02] px-3.5 py-1.5 text-sm font-medium tabular-nums text-slate-400/80 ring-1 ring-white/[0.03]">
                  {formatTime(now)}
                </span>
              </div>
            </div>
          </div>

          {/* Main content area */}
          <div className="px-8 py-7">{renderContent()}</div>

          {/* Status footer — music, timer */}
          {(musicLabel || timerRemaining) && (
            <div className="flex flex-wrap items-center gap-5 border-t border-white/[0.04] px-8 py-3.5">
              {musicLabel && (
                <span className="text-xs font-medium text-slate-600">
                  Music: {musicLabel}
                  {isPlaying ? ' ▶' : ''}
                </span>
              )}
              {timerRemaining && (
                <span className="ml-auto text-xs font-medium tabular-nums text-slate-600">
                  Time left: {timerRemaining}
                </span>
              )}
            </div>
          )}
        </div>
      </div>
    </div>
  )
}
