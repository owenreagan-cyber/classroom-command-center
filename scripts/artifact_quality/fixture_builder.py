...
def build_warn_whitespace_footer(path: Path) -> None:
    """Sparse lower page with isolated footer page number."""
    doc = _new_letter_doc()
    page = doc[0]
    page.insert_text((SAFE_X, SAFE_Y), "Section A: Short body text near the top.", fontsize=13)
    page.insert_text((SAFE_X, SAFE_Y + 30), "Complete the activity on the next page.", fontsize=12)
    # Footer sits in the physical bottom margin, outside the printable clip.
    page.insert_text((280, 770), "Page 1 of 3", fontsize=10)
    doc.save(path)
    doc.close()


def build_fail_clipping(path: Path) -> None:
    doc = _new_letter_doc()
    page = doc[0]
    page.insert_text((4, 400), "Essential content crosses the left safe margin.", fontsize=14)
    page.draw_line((0, 200), (40, 200), width=2.0)
    doc.save(path)
    doc.close()


def build_vector_only_diagram(path: Path) -> None:
    """Coordinate plane with grid lines and axes — no extractable text."""
    doc = _new_letter_doc()
    page = doc[0]
    origin_x, origin_y = 120, 520
    axis_len = 360
    page.draw_line(fitz.Point(origin_x, origin_y), fitz.Point(origin_x + axis_len, origin_y), width=1.5, color=(0, 0, 0))
    page.draw_line(fitz.Point(origin_x, origin_y), fitz.Point(origin_x, origin_y - axis_len), width=1.5, color=(0, 0, 0))
    for i in range(1, 10):
        x = origin_x + i * 36
        page.draw_line(fitz.Point(x, origin_y - 4), fitz.Point(x, origin_y + 4), width=0.8, color=(0.2, 0.2, 0.2))
        y = origin_y - i * 36
        page.draw_line(fitz.Point(origin_x - 4, y), fitz.Point(origin_x + 4, y), width=0.8, color=(0.2, 0.2, 0.2))
    page.draw_rect(fitz.Rect(origin_x + 40, origin_y - 280, origin_x + 320, origin_y - 40), width=1.0, color=(0, 0, 0))
    doc.save(path)
    doc.close()


def build_light_gray_lines(path: Path) -> None:
    doc = _new_letter_doc()
    page = doc[0]
    page.insert_text((SAFE_X, SAFE_Y), "Writing lines test", fontsize=14)
    gray = (0.72, 0.72, 0.72)
    for y in range(SAFE_Y + 60, SAFE_BOTTOM_Y - 40, 28):
        page.draw_line((SAFE_X, y), (520, y), color=gray, width=0.6)
    doc.save(path)
    doc.close()


def build_student_two_pages(path: Path) -> None:
    doc = fitz.open()
    for page_no in (1, 2):
        page = doc.new_page(width=LETTER[0], height=LETTER[1])
        page.insert_text((SAFE_X, SAFE_Y), f"Student Worksheet — Page {page_no}", fontsize=16)
        for i in range(1, 4):
            page.insert_text((SAFE_X, SAFE_Y + 40 + i * 36), f"{i + (page_no - 1) * 3}. Sample question.", fontsize=12)
        page.insert_text((500, SAFE_BOTTOM_Y), str(page_no), fontsize=10)
    doc.save(path)
    doc.close()


def build_key_three_pages(path: Path) -> None:
    doc = fitz.open()
    for page_no in (1, 2, 3):
        page = doc.new_page(width=LETTER[0], height=LETTER[1])
        page.insert_text((SAFE_X, SAFE_Y), f"Teacher Key — Page {page_no}", fontsize=16)
        page.insert_text((SAFE_X, SAFE_Y + 40), "Answer: sample teacher response.", fontsize=12)
        page.insert_text((500, SAFE_BOTTOM_Y), str(page_no), fontsize=10)
    doc.save(path)
    doc.close()


def build_student_layout(path: Path) -> None:
    doc = _new_letter_doc()
    page = doc[0]
    page.insert_text((SAFE_X, SAFE_Y), "Math Practice", fontsize=16)
    for i in range(1, 5):
        y = SAFE_Y + 50 + i * 48
        page.insert_text((SAFE_X, y), f"{i}. Solve: {i} + {i} = ___", fontsize=12)
        page.draw_rect(fitz.Rect(SAFE_X + 200, y - 14, 480, y + 28), width=0.5)
    doc.save(path)
    doc.close()


def build_key_reflow_warn(path: Path) -> None:
    """Teacher key with extra answer lines that increase visible ink vs student."""
    doc = _new_letter_doc()
    page = doc[0]
    page.insert_text((SAFE_X, SAFE_Y), "Math Practice — Key", fontsize=16)
    for i in range(1, 5):
        y = SAFE_Y + 50 + i * 48
        page.insert_text((SAFE_X, y), f"{i}. Solve: {i} + {i} = {i + i}", fontsize=12)
        page.insert_text((SAFE_X + 200, y + 14), f"Answer: {i + i}. Show regrouping if needed.", fontsize=11)
        page.draw_rect(fitz.Rect(SAFE_X + 200, y - 14, 480, y + 28), width=0.5)
    doc.save(path)
    doc.close()


def build_key_overflow_fail(path: Path) -> None:
    """Teacher key with overlapping answer text and an extra page."""
    doc = fitz.open()
    page = doc.new_page(width=LETTER[0], height=LETTER[1])
    page.insert_text((SAFE_X, SAFE_Y), "Math Practice — Key", fontsize=16)
    y = SAFE_Y + 50
    for i in range(1, 8):
        page.insert_text((SAFE_X, y), f"{i}. Answer block with extended teacher notes.", fontsize=12)
        page.insert_text((SAFE_X, y + 14), "Detailed solution steps that overlap the next question area.", fontsize=11)
        y += 28
    page = doc.new_page(width=LETTER[0], height=LETTER[1])
    page.insert_text((SAFE_X, SAFE_Y), "Overflow page 2", fontsize=14)
    doc.save(path)
    doc.close()


def build_twenty_page_mixed(path: Path) -> None:
    doc = fitz.open()
    for page_no in range(1, 21):
        page = doc.new_page(width=LETTER[0], height=LETTER[1])
        page.insert_text((SAFE_X, SAFE_Y), f"Mixed content page {page_no}", fontsize=14)
        for y in range(SAFE_Y + 50, SAFE_Y + 200, 24):
            page.draw_line((SAFE_X, y), (520, y), width=0.3)
        if page_no % 3 == 0:
            page.draw_rect(fitz.Rect(SAFE_X, 300, 520, 380), width=0.5)
            shade = fitz.Rect(SAFE_X + 4, 304, 516, 376)
            page.draw_rect(shade, fill=(0.94, 0.94, 0.94))
        page.insert_text((500, SAFE_BOTTOM_Y), str(page_no), fontsize=10)
    doc.save(path)
    doc.close()


def build_image_only_map(path: Path) -> None:
    """Raster map-like image with no text layer."""
    from PIL import Image
    import io

    doc = _new_letter_doc()
    page = doc[0]
    img = Image.new("RGB", (400, 300), (210, 225, 190))
    for x in range(0, 400, 40):
        for y in range(0, 300, 40):
            color = (180 + (x // 40) * 5, 200, 170 + (y // 40) * 3)
            for dx in range(40):
                for dy in range(40):
    ...
    buf.seek(0)
    page.insert_image(fitz.Rect(SAFE_X, SAFE_Y + 20, SAFE_X + 400, SAFE_Y + 320), stream=buf.read())
    doc.save(path)
    doc.close()


def build_vector_response_lines(path: Path) -> None:
    doc = _new_letter_doc()
    page = doc[0]
    for i in range(4):
        y = SAFE_Y + 60 + i * 70
        page.draw_rect(fitz.Rect(SAFE_X, y, 520, y + 50), width=0.6)
        for line_y in range(y + 12, y + 46, 10):
            page.draw_line((SAFE_X + 8, line_y), (512, line_y), width=0.3)
    doc.save(path)
    doc.close()


def build_page_number_only(path: Path) -> None:
    doc = _new_letter_doc()
    page = doc[0]
    page.insert_text((280, 760), "Page 1 of 1", fontsize=10)
    doc.save(path)
    doc.close()


def build_writing_lines_lower_page(path: Path) -> None:
    doc = _new_letter_doc()
    page = doc[0]
    page.insert_text((SAFE_X, SAFE_Y), "Notes section", fontsize=14)
    for y in range(400, SAFE_BOTTOM_Y - 80, 28):
        page.draw_line((SAFE_X, y), (520, y), width=0.6, color=(0.3, 0.3, 0.3))
    page.insert_text((280, 720), "Page 1 of 2", fontsize=10)
    doc.save(path)
    doc.close()


def ensure_phase2_fixtures(base: Path) -> None:
    """Deterministic Phase 2 edge-case fixtures (flat names under base/)."""
    builders = {
        base / "pass_worksheet.pdf": build_passing_worksheet,
        base / "warn_whitespace.pdf": build_warn_whitespace_footer,
        base / "fail_clipping.pdf": build_fail_clipping,
        base / "vector_only_diagram.pdf": build_vector_only_diagram,
        base / "light_gray_lines.pdf": build_light_gray_lines,
        base / "student_two_pages.pdf": build_student_two_pages,
        base / "key_three_pages.pdf": build_key_three_pages,
        base / "student_layout.pdf": build_student_layout,
        base / "key_reflow_warn.pdf": build_key_reflow_warn,
        base / "key_overflow_fail.pdf": build_key_overflow_fail,
        base / "twenty_page_mixed_content.pdf": build_twenty_page_mixed,
        base / "image_only_map.pdf": build_image_only_map,
        base / "vector_response_lines.pdf": build_vector_response_lines,
        base / "page_number_only.pdf": build_page_number_only,
        base / "writing_lines_lower_page.pdf": build_writing_lines_lower_page,
    }
